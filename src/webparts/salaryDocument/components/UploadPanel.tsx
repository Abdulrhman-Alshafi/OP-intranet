...
import { SalaryDocumentService, IUploadResult, IUploadJob } from '../../../services/SalaryDocumentService';
...
export const UploadPanel: React.FC<IUploadPanelProps> = (props) => {
  ...
  // PeoplePicker context object (memoized so the picker doesn't re-render constantly)
  ...
  const handleSingleUpload = async (): Promise<void> => {
    ...
    setSingleResult(undefined);
    try {
      await service.uploadSalaryDocument(
        libraryName,
        singleFile,
  ...
  const [bulkJobs, setBulkJobs] = useState<IUploadJob[]>([]);
  const [uploadResults, setUploadResults] = useState<IUploadResult[]>([]);
  ...
  const handleValidate = useCallback(async () => {
    ...
    // Missing: in Excel but not uploaded
    ...
    // Validate emails + resolve SP user IDs
    const invalidEmails: string[] = [];
    const resolvedJobs: IUploadJob[] = [];
    ...
    for (const row of uniqueRows) {
      ...
      if (!file) continue; // already flagged as missing
  ...
  const handleBulkUpload = async (): Promise<void> => {
    ...
    await service.uploadBulkDocuments(libraryName, bulkJobs, accountantsGroupId, (result) => {
